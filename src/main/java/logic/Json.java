    ...
    private static JsonNode replaceNullWithEmpty(JsonNode node) {
        ...
        if (node.isObject()) {
            ...
            while (fieldNames.hasNext()) {
                ...
                objectNode.set(fieldName, replaceNullWithEmpty(objectNode.get(fieldName))); // 递归处理内部字段
            }
            return objectNode;
        }
        if (node.isArray()) {
            ...
            for (int i = 0; i < arrayNode.size(); i++) {
                arrayNode.set(i, replaceNullWithEmpty(arrayNode.get(i))); // 递归处理数组中的每个元素
            }
            return arrayNode;
        }
